...
output_dir = os.path.join(script_dir, "..", "results", "rule_based_pv_bess")
csv_dir = os.path.join(output_dir, "csv")
excel_path = os.path.join(output_dir, f"rule_based_pv_bess_hourly_results.xlsx")
...
P_ch_max = P_dc_max = P_inv # Maximum charge/discharge pwoer
...
SOC_0 = E_bess * 0.5
annual_fixed_costs = (3.8568 + 4.18285 + 4.167) * 12 # 11.315 €/month * 12 ~ 146.4758 €/year
daily_fixed_costs = annual_fixed_costs / 366
CAPEX = (P_pv * 130 + E_bess * 350 + P_inv * 190) + 135 + 800 + 2000 + 2500
...
for t in range(T):
    ...
    cycle_t = p_dc_t / (E_bess * DoD)  # Battery cycle depth used in this hour, normalized to usable capacity
...
hourly_csv_path = os.path.join(csv_dir, f"rule_based_pv_bess_hourly_results.csv")
daily_csv_path = os.path.join(csv_dir, f"rule_based_pv_bess_daily_summary.csv")
hourly_results_df.to_csv(hourly_csv_path, index=False)
daily_summary_df.to_csv(daily_csv_path, index=False)
...
# === Logging Summary ===
logging.info(f"✅ Yearly rule_based resulst for PV + BESS are created.")
